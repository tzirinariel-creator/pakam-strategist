...
export function buildRecommendations(
  ...
  for (const [idx, { c, days }] of moedBCandidates.slice(0, 2).entries()) {
    ...
    recs.push({
      ...
      bodyHe: `קיבלת ${c.grade} ב${nameHe}. מועד ב׳ ${whenHe} — אפשרות לשפר. שימו לב: בישראל הציון האחרון קובע (לא הגבוה), אז שווה לגשת רק אם אתם בטוחים שתשתפרו.`,
  ...
  if (input.currentYear <= 1) {
    if (input.amiramScore === null) {
      recs.push({
        ...
        bodyHe: "ציון האמירנט קובע את תוכנית-האנגלית שלך (פטור או קורסי-רמה). שווה להוסיף אותו בהגדרות כדי לראות מה נדרש ועד מתי.",
  ...
  if (!input.hasFocusArea) {
    recs.push({
      ...
      bodyHe: `תחום המיקוד קובע ${CREDIT_REQUIREMENTS.FOCUS_AREA_MIN} ש״ס מהתואר — וגם את הסיווג שלך בשירות המדינה. כדאי לבחור מוקדם כדי לתכנן נכון.`,
