  ...
  const tooltip = (
    ...
    >
      <button
        ...
        className="absolute end-2 top-2 rounded-md p-1 text-foreground/60 transition-colors hover:bg-foreground/5 hover:text-foreground/70"
      >
        <X className="size-4" />
      </button>
      ...
      <div className="mt-3 flex items-center justify-between gap-2">
        <button
          type="button"
          onClick={back}
          className={cn("flex items-center gap-1 rounded-lg px-2.5 py-1.5 text-sm text-foreground/60 hover:bg-foreground/5", step === 0 && "pointer-events-none opacity-0")}
        >
...
export function TourReopenButton({ onClick }: { onClick: () => void }) {
  ...
  return (
    <button
      ...
      className="inline-flex items-center gap-1.5 rounded-lg border border-border/60 bg-card/40 px-2.5 py-1.5 text-xs font-medium text-foreground/60 transition-colors hover:border-foreground/25 hover:bg-foreground/5 hover:text-foreground/80"
    >
      <span
        aria-hidden
