...
export function StepWelcome({ onNext, selectedProgram, onProgramSelect }: StepWelcomeProps) {
  ...
  return (
    <div className="flex flex-col items-center text-center">
      ...
      {/* ==================================================================
          W2 — "מסך בחירת התוכנית שואל שאלה עם תשובה אחת"
          ==================================================================
          `ACTIVE_PROGRAMS` מכיל את פכ״מ בלבד, כרטיס המשפטים תמיד מושבת
          עם "בקרוב", והבחירה גם לא נשלחת לשרת (`data.program` לא בפיילוד
          ולא בסכמה של `updateProfile`). כלומר המסך הראשון אחרי ההרשמה
          שאל *"באיזו תוכנית אתם לומדים?"* — שאלה שאי־אפשר לענות עליה
          לא נכון, ושהתשובה לה נזרקת.
          לא הסרתי את הכרטיס: הוא אומר לאן זה הולך, וזו החלטה של אריאל.
          מה שכן — הוא הפסיק להתחזות לשאלה. הכותרת אומרת מה המצב.
          כשמשפטים ייפתח, זו תהיה שאלה אמיתית וגם התשובה תצטרך להישלח. */}
      {programs.length > 1 && (
