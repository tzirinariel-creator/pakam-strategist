...
export function RuleList({ results }: RuleListProps) {
  ...
  // Group rules by status: failed first, then warnings, then passed
  const { failedRules, warningRules, passedRules } = useMemo(() => {
    const failed: RegulationResult[] = [];
    const warning: RegulationResult[] = [];
    const passed: RegulationResult[] = [];

    for (const rule of results) {
      if (!rule.passed && rule.severity === "ERROR") {
        failed.push(rule);
      } else if (!rule.passed && rule.severity === "WARNING") {
        warning.push(rule);
      } else {
        passed.push(rule);
      }
    ...
    return { failedRules: failed, warningRules: warning, passedRules: passed };
  }, [results]);
