...
- ⚠️ **STILL OPEN — the 93 vs 103 mandatory-credits gap:** official 103 confirmed, but our catalog sums mandatory to 93 → ~10 ש"ז of mandatory courses missing. The full named list couldn't be fetched (the Yedion page tcid=5904 is JS-rendered + bot-blocked). To close it: pull the Yedion's internal JSON endpoint (open the page with the Network tab, filter Fetch/XHR) or ask the PPE secretariat.
